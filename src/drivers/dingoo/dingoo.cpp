...
extern uint8 PAL;
int numframes;
if (PAL) numframes=50; else numframes=60;
if (fpsc > numframes+1) {
fpsc=0;
/*8uts("fpsc ");
printf("%d",fpsc);
...
int ci;
int fpsu;
if (skip > numframes) skip = 0;
fpsu=floor(numframes / frameskip + 0.5);
for (ci=1;ci<=frameskip;ci++) {
if (fpsu * ci == skip) {fskipc=1; break;} else fskipc=0;
}
skip++;
...
FCEUD_DriverReset()
{
	...
	if(InitVideo(GameInfo) < 0) return 0;
	inited|=4;
...
int FCEUD_LoadMovie(const char *name, char *romname)
{
    ...
    {
        char tmp[128];
        if (!FCEUD_ConvertMovie(name, tmp)) return 0;
        s = std::string(tmp);
    }
	...
	if(s.find(".fm2") != std::string::npos)
	{
        ...
        if( !RunFileBrowser(NULL, romname, types, "Select movie's rom file") ) {
   		...
   		if( LoadGame(romname) != 1 ) return -1;
