...
static void soundrate_update(unsigned long key)
{
    ...
    for(i=0;i<7;i++) if (val == rates[i]) break;

    if (key & DINGOO_RIGHT) i = i < 6 ? i+1 : 6;
...
static void priority_update(unsigned long key)
{
    ...
    if (key & DINGOO_RIGHT) val = val < 19 ? val+1 : 19;
    if (key & DINGOO_LEFT) val = val > -20 ? val-1 : -20;
...
static void volume_update(unsigned long key)
{
    ...
    if (key & DINGOO_RIGHT) val = val < 256 ? val+1 : 256;
